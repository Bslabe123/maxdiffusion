...
def create_step_to_timestamp(mllog_path: str):
  step_to_timestamp = {}
  with tf.io.gfile.GFile(mllog_path, 'r') as f:
    for line in f:
      if line.startswith(":::MLLOG") and "checkpoint" in line:
        ...
        step, timestamp = log_dict["metadata"]["step_num"], log_dict['time_ms']
        step_to_timestamp[step] = timestamp

  return step_to_timestamp
...
def main():
  ...
  step_to_timestamp = create_step_to_timestamp(args.mllog_path)
  ...
  for row in df.itertuples():
    if row.fid <= args.target_fid and row.clip >= args.target_clip:
      ...
      success_timestamp = step_to_timestamp[success_step]
      mllog_utils.timestamp_fid(row.fid, success_timestamp, row.step)
      mllog_utils.timestamp_clip(row.clip, success_timestamp, row.step)
      print(f"Found checkpoint matching targets at step {row.step}: {args.target_fid=} {args.target_clip=}")
      break
  else:
    print(f"Could not find checkpoint matching targets: {args.target_fid=} {args.target_clip=}")

  if success_timestamp is not None:
    mllog_utils.timestamp_run_stop_success(success_step, success_timestamp)
  else:
